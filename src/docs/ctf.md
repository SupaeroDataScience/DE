...
The CTF server's IP address is [`34.163.57.143`](http://34.163.57.143/). You can see a leaderboard there and it is the address for submitting answers. The first way we'll look at submitting answers is with `curl` in Linux.
...
```bash
curl -X POST 'http://34.163.57.143/' \
    -d 'number=1' \
...
```bash
curl 'http://34.163.57.143/user/d.wilson'
```
...
```bash
curl 'http://34.163.57.143/answers/'
```
...
For the ssh section, you will connect to the CTF server to answer questions about the remote environment. Your public key must be uploaded to the git repository above to get access to the server. You will use the corresponding private key to access the server. Your user on the server is `ctf` and the IP is the same as the CTF webserver: `34.163.57.143`.
